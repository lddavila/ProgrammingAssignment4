...
public class backEnd {
     ...
     */
    public static Customer findAccount(String name, PersonDatabase databaseIn){
    ...
    public static void deposit(String dep, Customer person, double dammount){
        switch (dep) {
            case "1":
    ...
    public static void withdraw(String with,double wammount, Customer person){
        switch (with) {
            case "1":
                if (person.CheckingAcc.get_Starting_Balance()>= wammount) {
                    person.CheckingAcc.withdrawal(wammount);

                    System.out.println(person.getFirstName() + " withdrew $" + wammount + " from checking new balance is $" + person.CheckingAcc.get_Starting_Balance());
                    try {//writes transaction to log
                        //FileWriter out = new FileWriter("transLog.txt", false);
                        writeUserActions.logUserAction(person.getFirstName()+ " withdrew $" + wammount + " from checking new balance is $" + person.CheckingAcc.get_Starting_Balance());
                        writeUserActions.logUserAction("\r\n");
                ...
                break;
            case "2":
                if(person.SavingAcc.get_Starting_Balance()>=wammount) {
                    person.SavingAcc.withdrawal(wammount);

                    System.out.println(person.getFirstName() + " withdrew $" + wammount + " from saving new balance is $" + person.SavingAcc.get_Starting_Balance());
                    try {//writes transaction to log
                        //FileWriter out = new FileWriter("transLog.txt", false);
                        writeUserActions.logUserAction(person.getFirstName() + " withdrew $" + wammount + " from saving new balance is $" + person.SavingAcc.get_Starting_Balance());
                        writeUserActions.logUserAction("\r\n");
    ...
    public static int transfer(Account checkingIn, PersonDatabase people) {
        ...
        while (actionCompleted == false) {
            try {
                ...
                Customer otherUser = findAccount(userInput, people);
    ...
    public static void invalidCommand(Customer accountIn){
     ...
     */
    public static void userExit(Customer user){
     ...
     */
    public static int secondaryMenu(Customer user, int actionIn, PersonDatabase people) {
            ...
            try {
                ...
                switch (actionIn) {
                    ...
                    case 1:
                        ...
                        //handles withdrawals
                    case 2:
                        System.out.println("What Account would you like to withdrawal from? (Enter -1 to Exit)");
                        System.out.println("1.) Checking");
                        System.out.println("2.) Savings");
                        accountAction = userInput.nextLine();
                        if (accountAction.equals("-1"))
                            return 0;
                        if(accountAction.equals("1") || accountAction.equals("2")) {
                            System.out.println("How much would you like to Withdrawal? (Enter -1 to Exit)");
                            amount = userInput.nextDouble();
                            if (accountAction.equals("-1"))
                                return 0;
                            try {
                                withdraw(accountAction, amount, user);
                                return 0;
                            } catch (Exception e) {
                                System.out.println("Sorry you either entered an invalid amount or an invalid action, please try again.");
                                writeUserActions.logUserAction(user.getFirstName() + user.getLastname() + "entered an invalid action/amount.");
     ...
     */
    public static int printInitialMenu(Customer user, PersonDatabase people){
        Scanner userInput = new Scanner(System.in);
        try {
